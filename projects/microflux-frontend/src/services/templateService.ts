...
export const TEMPLATES: WorkflowTemplate[] = [
...

  {
    ...
    nodes: [
      ...
      },
      {
        ...
        category: 'notification',
        config: { chatId: '', message: 'MicroFlux workflow run: swap + sheet log completed.' },
  ...
  // ── AUTOMATION ──────────────────────────────
  {
    ...
    description:
      'Manual trigger → ALGO/USD quote → filter on price ≥ 0 → optional 0.05 ALGO payment → Telegram notification (link wallet with /link in the bot, or set chatId on the node). No server cron; scheduling is illustrative only.',
    category: 'automation',
    tags: ['automation', 'schedule', 'payment', 'telegram'],
    difficulty: 'intermediate',
    ...
    nodes: [
      ...
      },
      {
        ...
        category: 'notification',
        config: { chatId: '', message: 'Scheduled payment workflow completed' },
...

  {
    ...
    name: 'Webhook → Action',
    description:
      'Webhook trigger → filter (status not empty) → send_payment → HTTPS callback via server proxy. Use https://httpbin.org/post for the callback URL (POST body is JSON).',
    category: 'automation',
    tags: ['webhook', 'automation', 'http'],
    difficulty: 'intermediate',
    estimatedGas: '0.001 ALGO',
    author: 'MICROFLUX-X1',
    nodes: [
      ...
      },
      {
        ...
        config: {
          ...
          method: 'POST',
          headers: {},
          body: { ok: true, source: 'microflux', note: 'webhook template callback' },
        },
        position: { x: 890, y: 200 },
      },
    ],
